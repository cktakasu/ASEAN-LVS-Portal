...
function T3RegulatoryGateway(): React.JSX.Element {
  return (
    <>
      {/* 電力基本仕様セクション */}
      <section className="content-block fade-in">
        <p className="section-kicker">POWER SYSTEM SPECIFICATIONS</p>
        <h2 style={{ fontSize: "28px" }}>電力基本仕様</h2>
        <p className="section-subline">Malaysia Low Voltage Power System</p>
        <article className="reference-block">
          <h3>基本仕様</h3>
          <div className="table-wrap">
            <table className="requirements-table" style={{ maxWidth: "800px", margin: "0 auto" }}>
              <tbody>
                {POWER_INFO.map((item, index) => (
                  <tr key={`power-${index}`}>
                    <td style={{ width: "35%", fontWeight: 600, backgroundColor: "#f8f9fa", padding: "12px 16px" }}>
                      {item.label}
                    </td>
        ...
        <article className="reference-block">
          ...
          <div className="table-wrap">
            <table className="requirements-table" style={{ maxWidth: "800px", margin: "0 auto" }}>
              <tbody>
                {REGIONAL_DIFF.map((item, index) => (
                  <tr key={`region-${index}`}>
                    <td style={{ width: "30%", fontWeight: 600, backgroundColor: "#f8f9fa", padding: "12px 16px" }}>
                      {item.label}
                    </td>
                    <td style={{ padding: "12px 16px", fontSize: "0.9rem", lineHeight: "1.6" }}>
                      {item.value}
                    </td>
                  </tr>
                ))}
          ...
          <p className="country-procedure" style={{ marginTop: "16px" }}>
            <strong>注意：</strong>
            サバ・サラワク（ボルネオ島）は半島マレーシアと異なる電力会社が管轄しており、規格・認証要件が異なる場合があります。
          </p>
        </article>
      </section>
